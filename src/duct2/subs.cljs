...
(defn sorted-tasks
  [db _]
  (:tasks db))
(reg-sub :sorted-tasks sorted-tasks)
...
(reg-sub
  ...
  (fn [query-v _]
    (subscribe [:sorted-tasks]))    ;; returns a single input signal
  ...
  ;; function above: (subscribe [:sorted-tasks])
  ;;
...
(reg-sub
  ...
  (fn [[tasks showing] _]   ;; that 1st parameter is a 2-vector of values
